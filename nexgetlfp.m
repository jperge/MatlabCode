...
% SigIndx = signalselector(P);
% if ~any(SigIndx),
%     return
% end
% neuronchans = NEURO.NeuronInfo.Channel;
% P.signal = lfplabels(neuronchans(SigIndx));

if ~isfield(P, 'signal') && ~isfield(P, 'channel'),
    P.signal = lfplabels;
elseif ~isfield(P, 'signal') && isfield(P, 'channel'),
    if ~isempty(setdiff(P.channel, lfpchans)),
        error('One or more specified channels not found');
    end
    indx = ismember(lfpchans, P.channel);
    P.signal = lfplabels(indx);
elseif ischar(P.signal),
    P.signal = {P.signal};
else
    P.signal = lfplabels(P.signal);
end
